...
namespace DushuBus.Database.Sqlite.Platform
{
    public class SQLiteApiWinRT : ISQLiteApi
    {
        ...
        {
            var dbStatement = (DbStatement)stmt;
            int length = this.ColumnBytes(stmt, index);
        ...
        {
            return this.ColumnBlob(stmt, index);
        }
        ...
        {
            var dbStatement = (DbStatement)stmt;
            IntPtr internalStmt = dbStatement.InternalStmt;
        ...
        {
            public DbHandle(IntPtr internalDbHandle)
                : this()
            {
                this.InternalDbHandle = internalDbHandle;
            }

            public IntPtr InternalDbHandle { get; set; }

            public bool Equals(IDbHandle other)
            {
                return other is DbHandle && this.InternalDbHandle == ((DbHandle)other).InternalDbHandle;
            }
        }

        private struct DbStatement : IDbStatement
        {
            public DbStatement(IntPtr internalStmt)
                : this()
            {
                this.InternalStmt = internalStmt;
            }

            internal IntPtr InternalStmt { get; set; }

            public bool Equals(IDbStatement other)
            {
                return (other is DbStatement) && ((DbStatement)other).InternalStmt == this.InternalStmt;
            }
        }
    }
